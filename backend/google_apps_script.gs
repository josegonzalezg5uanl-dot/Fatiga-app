 ...
 * Ver archivo completo para instrucciones detalladas
 * =====================================================
 */
...
function doPost(e) {
  try {
    Logger.log('📥 Petición recibida: ' + e.postData.contents);
    
    const data = JSON.parse(e.postData.contents);
    const userId = data.user_id;
    const fatigueLevel = data.fatigue_level;
    const capacityLevel = data.capacity_level;
    const suspensionReason = data.suspension_reason;
    ...
    if (fatigueLevel === undefined || fatigueLevel < 0 || fatigueLevel > 100) {
      throw new Error('Fatiga debe estar entre 0-100');
    }
    
    if (capacityLevel === undefined || capacityLevel < 0 || capacityLevel > 100) {
      throw new Error('Capacidad debe estar entre 0-100');
    }
    
    if (suspensionReason === undefined || suspensionReason < 1 || suspensionReason > 4) {
